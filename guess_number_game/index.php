...
<div class="pb-5 px-10">
    <div class="mt-12 ml-12 flex">
        <form class="w-[60%]" method="post" action="guessChecker.php">
            ...
            <div class="w-[350px]">
                <?php if (isset($_SESSION['attempts'])) {
                    echo '<div class="mx-auto p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
                     role="alert">Количество попыток: ' . $_SESSION['attempts'] . '</div>';
                }
                ?>
            </div>
            <label for="large-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Введите
                число</label>
            <input name="guessNumber" type="text" id="large-input"
        ...
        <div class="w-[30%] pt-5">
            ...
            <?php if (isset($_SESSION['message'])) {
                echo '<div class="mx-auto p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400"
                 role="alert">' . $_SESSION['message'] . '</div>';
                unset($_SESSION['message']);
            }
            ?>
            <?php if (isset($_SESSION['success'])) {
                echo '<div class="w-full mx-auto p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400"
